...
function renderQueueRowActions(item) {
    ...
    if (status === "queued") {
        return `<div class="ui compact basic icon buttons">
          <a class="ui button" href="/encoding/setup?id=${encodeURIComponent(item.id)}&origin=queue" title="Update queue item" aria-label="Update queue item">
...
function calculateRemainingProcessingMs(activeItem, worker, progress) {
    ...
    const remainingMediaMs = Math.max(0, durationMs - outTimeMs);
    if (!remainingMediaMs) {
        return 0;
    }
